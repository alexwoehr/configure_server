...
readonly search_value='lockfile=/var/lock/subsys/$prog'
readonly new_value='lockfile=/var/lock/subsys/mysql-container/mysqld # line changed by install_mysql_chroot.sh'
change_line=$(grep --line-number --fixed-strings --line-regexp -e "$search_value" etc/init.d/mysqld | cut -d: -f1 | head -1 )
sed --in-place "${change_line}c\\\n$new_value" etc/init.d/mysqld

# Step down privileges and initiate mysql daemon
cd "$CHROOT_JAIL_DIR" \
&& chroot --userspec="$USER" "$CHROOT_JAIL_DIR" service mysqld start

# TODO: Create options for--
# - do not create dev (? does mysql need dev and proc?)
